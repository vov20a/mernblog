...
import { IAllCategories } from '../../../features/categories/categoriesApiSlice';
...
const TAGS_REGEX = /^[A-z\s]{3,}$/;
const TEXT_REGEX = RegExp(/<[^>]*>/g);

const EditPostFormAuthor = ({ post, catsData, catsArray }: EditPostProps) => {
  ...
  const [tags, setTags] = useState<string>(post?.tags.length ? post?.tags?.join(' ') : ''); //слова идут через пробел в одной строке
  ...
  const canSave =
    [validTitle, category, author !== '', text !== '', tagsArray?.length].every(Boolean) &&
  ...
  const validCatsClass = !Boolean(Object.keys(catsArray).length) ? 'form__input--incomplete' : '';
  const validTagsClass = !validTags ? 'form__input--incomplete' : '';
  const validUsersClass = !author ? 'form__input--incomplete' : '';
            ...
            <div className="form-group">
              ...
              <div className={`input-group  ${validImageClass}`}>
                {oldImage ? (
                  ...
                  >
                    <img width="37px" src={oldImage as string} alt="Preview" />
                    <i
...
export default EditPostFormAuthor;
