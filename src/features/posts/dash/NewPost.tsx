...
const NewPost = () => {
  ...
  const [author, setAuthor] = useState<string>(users ? users[0].id : '');
  ...
  useEffect(() => {
    setValidAuthor(AUTHOR_REGEX.test(author));
  }, [author]);
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      setCategory('');
      setImageUrl('');
      navigate('/dash', { state: { successNewPost: isSuccess, messageNewPost: data?.message } });
    }
  ...
  const canSave =
    [validTitle, validAuthor, text !== '', imageUrl !== '', tagsArray?.length].every(Boolean) &&
    !isLoading;
  ...
  const validAuthorClass = !validAuthor ? 'form__input--incomplete' : '';
